...
## [Unreleased]
### Added
- NFS-e: **Cód. Tributação Nacional padrão** configurável (Notas Fiscais → Configurações, default `010701`), aplicado na emissão automática (com boleto) e como sugestão no wizard quando o serviço não tem código — antes a emissão sem código falhava. O **Cód. Tributação Municipal** é opcional e **não tem default fixo** (um valor arbitrário como `100` é rejeitado pelo schema — erro E1235); só é enviado quando preenchido com o código válido do município. Requer rodar `updates/update_nfe_ctrib_padrao.sql`.
...
- Gatilhos de notificação: agora é possível **criar** e **excluir** gatilhos (antes só editar os pré-definidos), na tela Configurações → Notificações / Gatilhos. Cada gatilho define evento, canais (e-mail/WhatsApp), destinatários e template; no WhatsApp, vários gatilhos ativos do mesmo evento disparam (com dedupe por destinatário). Novo campo **"Nº de notificação (WhatsApp)" por cliente** no cadastro — se preenchido, as notificações do cliente vão para ele; se vazio, usa o Celular (fallback). Requer rodar `updates/update_cliente_whatsapp_notificacao.sql`.
