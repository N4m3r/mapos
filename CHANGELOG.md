...
## [Unreleased]
### Added
- NFS-e: códigos de tributação padrão configuráveis (Notas Fiscais → Configurações): **Cód. Tributação Nacional padrão** (default `010701`) e **Cód. Tributação Municipal padrão** (default `100`). São aplicados na **emissão automática (com boleto)** quando o serviço não tem código, e usados como sugestão no wizard. Antes, a emissão sem código no serviço falhava. Requer rodar `updates/update_nfe_ctrib_padrao.sql` (opcional — sem ele, usa os defaults embutidos 010701/100).
